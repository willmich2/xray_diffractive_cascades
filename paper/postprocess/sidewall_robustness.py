...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate sidewall-smearing robustness from an N_sweeps run.")
    parser.add_argument(
        "--base-id",
...
if __name__ == "__main__":
    ...
    print(
        f"sidewall_robustness: Using {n_gpus} GPU(s), {workers_per_gpu} worker(s) per GPU, {max_workers} total workers",
        flush=True,
    )
    ...
    save_path = f"{args.data_dir}/sidewall_robustness_results_{args.base_id}_{save_time}.npz"
    np.savez(
        save_path,
